...
* [OPENVPN/SERVER](#openvpnserver)
  ...
  * [Create OpenVPN Server](#1-create-openvpn-server)
  * [Delete OpenVPN Server](#2-delete-openvpn-server)
  * [Read OpenVPN Server](#3-read-openvpn-server)
  * [Update OpenVPN Server](#4-update-openvpn-server)
...
* [SYSTEM/CERTIFICATE](#systemcertificate)
  ...
  * [Update System Certificates](#4-update-system-certificates)
...
```bash
Method: POST
URL: https://{{$hostname}}/api/v1/services/openvpn/server
```
...
| Key | Type | Description |
| --- | ------|-------------|
| mode | string | Set the server mode option. Options are `p2p_tls`, `p2p_shared_key`, `server_tls`, `server_user` and `server_tls_user`. The default setting is `p2p_tls`. (optional) |
| protocol | string | Set protocol. Options are `udp4`, `udp6`, `tcp4`, `tcp6`, `udp` and `tcp`. The default setting is `udp4`. (optional) |
| authmode | string | Backend for authentication, This is only necessary if you have specified the `mode` to `server_user` or `server_tls_user`. The default setting is `Local Database` (optional). |
| dev_mode | string | Set Device mode. Options are `tun` and `tap`. The default setting is `tun`. (optional) |
| interface | string | The interface or Virtual IP address where OpenVPN will receive client connections. The default setting is `wan`. (optional) |
| local_port | string or integer | The port used by OpenVPN to receive client connections. Omitting this will default to next available port. (optional) |
...
| tls | string | Paste the TLS key. This is not necessary for mode `p2p_shared_key`. Options are `""` for automatic TLS key generation and a raw TLS key. Omitting disable TLS configuration. _Note: Import shared key in PEM format. `-----BEGIN OpenVPN Static key V1-----[A bunch of random-looking base64-encoded data]-----END OpenVPN Static key V1-----`. (optional) |
| tls_type | string | Enter TLS Key Usage Mode. This is not necessary for mode `p2p_shared_key`. Options are `auth` and `crypt`. The default setting is `auth`. (optional) |
| tlsauth_keydir | string | Enter TLS keydir direction. This is not necessary for mode `p2p_shared_key`. Options are `default`, `direction-0`, `direction-1` and `both-directions` . The default setting is `default`. (optional) |
| shared_key | string | Enter Shared Key. This is only mandatory for mode `p2p_shared_key`. Options are `""` for automatic shared key generation and a raw shared key. _Note: Import shared key in PEM format. `-----BEGIN OpenVPN Static key V1-----[A bunch of random-looking base64-encoded data]-----END OpenVPN Static key V1-----`. |
| caref | string | Specify the unique reference ID of the certificate signing authority. |
| crlref | string | Specify the unique reference ID of the certificate revocation list. The default setting is `none`. (optional) |
| certref | string | Specify the unique reference ID of the certificate. |
| ocspcheck | boolean | Whether to check client certificates with OCSP. (optional) |
| ocspurl | string | Enter OCSP URL. This is only necessary for `ocspcheck`. The default setting is `""`. (optional) |
| dh_length | string | Enter DH parameter length. Options are `1024`, `2048`, `3072`, `4096`, `6144`, `7680`, `8192`, `15360`, `16384` and `ECDH Only`. The default setting is `2048`. (optional) |
| ecdh_curve | string | Enter ECDH curve. Options are `none`, `prime256v1`, `secp384r1` and `secp521r1`. The default setting is `none`. (optional) |
| cert_depth | string | Enter Certificate depth. Options are `none`, `one`, `two`, `three`, `four` and `five`. The default setting is `one`. (optional) |
| strictusercn | boolean | Enforce Strict User-CN matching. This is only necessary for mode `server_tls_user`. Omitting will disable. (optional) |
| ncp_enable | boolean | Enable Data Encryption Negotiation. Disabling this feature is deprecated. Omitting will enable. (optional) |
| data_ciphers | string | Enter comma sepereted data encryption algorithms (e.g. `AES-256-GCM,AES-128-GCM,CHACHA20-POLY1305`). Data encryption algorithms will be truncated to 19.The default setting is `AES-256-GCM,AES-128-GCM,CHACHA20-POLY1305`. (optional) |
| data_ciphers_fallback | string | Enter fallback data encryption algorithm. The default setting is `AES-256-CBC`. (optional) |
| digest | string | Enter auth digest algorithm. Options are `SHA256`, `BLAKE2b512`, `BLAKE2b256`, `MD4`, `MD5`, `MD5-SHA1`, `MDC2`, `RIPEMD160`, `SHA1`, `SHA224`, `SHA256`, `SHA3-224`, `SHA3-256`, `SHA3-384`, `SHA3-512`, `SHA384`, `SHA512`, `SHA512-224`, `SHA512-256`, `SHAKE128`, `SHAKE256`, `SM3`, `whirlpool` and `none`. The default setting is `SHA256`. (optional) |
| engine | string | Enter Hardware crypto. Options are `none` and any enabled hardware crypto. The default setting is `none`. (optional) |
...
With net30 topology, the first network address of the /30 is assumed to be the server address and the second network address will be assigned to the client. NOTE: Using a tunnel network and server bridge settings together is not allowed. |
...
Enter the client IPv6 address and prefix. The prefix must match the IPv6 Tunnel Network prefix on the server. NOTE: Using a tunnel network and server bridge settings together is not allowed. |
| serverbridge_dhcp | boolean | Allow clients on the bridge to obtain DHCP. This is only available for mode `server_tls` , `server_user` or `server_tls_user` and dev_mode `tap`. NOTE: Using a tunnel network and server bridge settings together is not allowed. (optional) |
| serverbridge_interface | string | The interface to which this TAP instance will be bridged. This is not done automatically. This interface must be assigned and the bridge created separately. This setting controls which existing IP address and subnet mask are used by OpenVPN for the bridge. Setting this to `none` will cause the Server Bridge DHCP settings below to be ignored. This is only available for mode `server_tls` , `server_user` or `server_tls_user` and dev_mode `tap`. NOTE: currently support `wan`, `lan` and `none`. (optional) |
| serverbridge_routegateway | boolean | When omitting the IPv4 Tunnel Network for a bridge, connecting clients cannot automatically determine a server-side gateway for IPv4 Local Network(s) or Redirect IPv4 Gateway traffic. When enabled, this option sends the IPv4 address of the selected Bridge Interface to clients which they can then use as a gateway for routing traffic outside of the bridged subnet. OpenVPN does not currently support this mechanism for IPv6. This is only available for mode `server_tls` , `server_user` or `server_tls_user` and dev_mode `tap`. (optional) |
| serverbridge_dhcp_start | string | When using TAP mode as a multi-point server, a DHCP range may optionally be supplied to use on the interface to which this TAP instance is bridged. If these settings are left blank, DHCP will be passed through to the LAN, and the interface setting above will be ignored. This is only available for mode `server_tls` , `server_user` or `server_tls_user` and dev_mode `tap`. (optional) |
| serverbridge_dhcp_end | string | When using TAP mode as a multi-point server, a DHCP range may optionally be supplied to use on the interface to which this TAP instance is bridged. If these settings are left blank, DHCP will be passed through to the LAN, and the interface setting above will be ignored. This is only available for mode `server_tls` , `server_user` or `server_tls_user` and dev_mode `tap`. (optional) |
...
NOTE: Remember to add these subnets to the IPv4 Remote Networks list on the corresponding OpenVPN server settings. This is only available for mode `p2p_tls ` or `p2p_shared_key`. (optional) |
...
NOTE: Remember to add these subnets to the IPv6 Remote Networks list on the corresponding OpenVPN server settings. This is only available for mode `p2p_tls ` or `p2p_shared_key`. (optional) |
| redirect_gateway | boolean | Force all client generated traffic through the tunnel. (optional) |
| redirect_gateway6 | boolean | Force all client generated traffic through the tunnel. (optional) |
| concurrent_connections | string or integer | Specify the maximum number of clients allowed to concurrently connect to this server. (optional) |
| allow_compression | string | Allow compression to be used with this VPN instance. Options are `asym`, `no` and `yes`. The default setting is `no` (optional) |
| compression | string | Compression to be used with this VPN instance. This is only necessary for `allow_compression` set to `yes` or `asym`. Options are `none`, `stub`, `stub-v2`, `lz4`, `lz4-v2`, `lzo`, `noadapt`, `adaptive`, `yes` and `no`. Compression can potentially dangerous and insecure. Deprecated. Compress tunnel packets using the LZO algorithm. The default setting is `none` (optional) |
| compression_push | boolean | Push the selected Compression setting to connecting clients. This is only necessary for `allow_compression` set to `yes` or `asym`. (optional) |
| passtos | boolean | Allow communication between clients connected to this server. (optional) |
| client2client | boolean | Whether to disable this override without removing it. (optional) |
| duplicate_cn | boolean | Allow multiple concurrent connections from the same user. This is only necessary for mode `p2p_shared_key`. (optional) |
| dynamic_ip | boolean | Allow connected clients to retain their connections if their IP address changes. (optional) |
| topology | string | Specifies the method used to supply a virtual adapter IP address to clients when using TUN mode on IPv4. Options are `subnet` and `net30`. The default setting is `subnet`. (optional) |
| dns_domain | string | Domain name for DNS to provide to the client. (optional) |
...
| push_blockoutsidedns | boolean | Make Windows 10 Clients Block access to DNS servers except across OpenVPN while connected, forcing clients to use only VPN DNS servers. (optional) |
| push_register_dns | boolean | Run "net stop dnscache", "net start dnscache", "ipconfig /flushdns" and "ipconfig /registerdns" on connection initiation. This only availabe for mode `server_tls`, `server_user` and `server_tls_user`. (optional) |
| inactive_seconds | string or integer | Causes OpenVPN to exit after n seconds of inactivity on the TUN/TAP device. `0` disables this feature. The default setting is `300`. (optional) |
| ping_method | string | Enter ping method. Options `keepalive` and `ping`. The default setting is `keepalive`. (optional) |
| keepalive_interval | string or integer | Enter keepalive interval. The default setting is `10`. This only available for `keepalive` method. (optional) |
| keepalive_timeout | string or integer | Enter keepalive timeout. The default setting is `60`. This only available for `keepalive` method. (optional) |
| ping_seconds | string or integer | Enter ping seconds. This is only necessary for `ping` method. The default setting is `10`. (optional) |
| ping_action_seconds | string or integer | Enter ping action seconds. This is only necessary for `ping` method. The default setting is `60`. (optional) |
| ping_push | boolean | Push ping to VPN client. This is only necessary for `ping` method. (optional) |
| ping_action_push | boolean | Push ping-restart/ping-exit to VPN client. This is only necessary for `ping` method. (optional) |
| ping_action | string | Exit or restart OpenVPN after timeout from remote. Options `ping_exit` and `ping_restart`. This is only necessary for `ping` method. The default setting is `ping_restart`. (optional) |
| username_as_common_name | boolean | Use the authenticated client username instead of the certificate common name (CN). This is only necessary for mode `server_user` or `server_tls_user`. The default setting is `false`. (optional) |
| udp_fast_io | boolean | Use fast I/O operations with UDP writes to tun/tap. Experimental. The default setting is `false`. (optional) |
| exit_notify | string | Send an explicit exit notification to connected clients/peers when restarting or shutting down, so they may immediately disconnect rather than waiting for a timeout. In SSL/TLS Server modes, clients may be directed to reconnect or use the next server. In Peer-to-Peer Shared Key or with a /30 Tunnel Network, this value controls how many times this instance will attempt to send the exit notification. Options `once`, `twice`, `none` and `""`. The default setting is `once`. (optional) |
| sndrcvbuf | string | Configure a Send and Receive Buffer size for OpenVPN. Options `64KiB`, `128KiB`, `256KiB`, `512KiB`, `1MiB`, `2MiB` and `default`. The default setting is `default`. (optional) |
| ntp_servers | string | A comma-separated list of no more than 2 NTP server IP addresses. Any more than the first two are ignored. (optional) |
| netbios_enable | boolean | Enable NetBIOS over TCP/IP. If this option is not set, all NetBIOS-over-TCP/IP options (including WINS) will be disabled. (optional) |
| netbios_node_type | string | Possible options: b (broadcasts), p (point-to-point name queries to a WINS server), m (broadcast then query name server), and h (query name server, then broadcast). This parameter takes a single letter either b, p, m, or h. Any other string will result in this option being set to none. (optional) |
| netbios_scope | string | A NetBIOS Scope ID provides an extended naming service for NetBIOS over TCP/IP. The NetBIOS scope ID isolates NetBIOS traffic on a single network to only those nodes with the same NetBIOS scope ID. (optional) |
| create_gw | string | If you assign a virtual interface to this OpenVPN server, this setting controls which gateway types will be created. Options are `v4only`, `v6only` and `both`. The default setting is `both`. (optional) |
| verbosity_level | string | Each level shows all info from the previous levels. Level `3` is recommended for a good summary of what's happening without being swamped by output. Maximum level `11`. The default setting is `default`, equivalent to level `1`. (optional) |
| wins_servers | string | A comma-separated list of no more than 2 WINS server IP addresses. Any more than the first two are ignored. (optional) |
| disable | boolean | Whether to disable this server. (optional) |
...
```js        
{
    "mode": "server_tls_user",
        "caref": "61c7fbce3a351",
    "crlref": "61c467cd2fd4f",
    "certref": "61c4121240420",
    "custom_options": "ifconfig-push xxx.xxx.xxx.xxx 255.255.255.0;",
    "disable": true,
    "protocol": "tcp",
    "interface": "lan",
    "description": "A server, that is specific...",
...
| Key | Type | Description |
| --- | ------|-------------|
| vpnid | string or integer | VPN id. This id indicates the id of the server you want to delete. This id is static and not an index of an entry in the config array. Best to first get all servers and get the id of the one you want to delete before calling this endpoint. |
...
| Key | Type | Description |
| --- | ------|-------------|
| vpnid | string or integer | VPN id. This id indicates the id of the server you want to update. This id is static and not an index of an entry in the config array. Best to first get all servers and get the id of the one you want to delete before calling this endpoint. |
| mode | string | Set the server mode option. Options are `p2p_tls`, `p2p_shared_key`, `server_tls`, `server_user` and `server_tls_user`. The default setting is `p2p_tls`. (optional) |
| protocol | string | Set protocol. Options are `udp4`, `udp6`, `tcp4`, `tcp6`, `udp` and `tcp`. The default setting is `udp4`. (optional) |
| authmode | string | Backend for authentication, This is only necessary if you have specified the `mode` to `server_user` or `server_tls_user`. The default setting is `Local Database` (optional). |
| dev_mode | string | Set Device mode. Options are `tun` and `tap`. The default setting is `tun`. (optional) |
...
<<<<<<< HEAD
>>>>>>> 14b5da3 (Updated contributing guidelines about updating documentation, removed endpoint documentation from README.md and prompt directing users to new Swagger docs after installation)
=======
>>>>>>> 4e85721 (Minor README formatting fixes)
=======
...
<<<<<<< HEAD
>>>>>>> cad3fc6 (Matching README to OpenAPI docs)
=======
=======
# Rate limit
There is no limit to API calls at this time but is important to note that pfSense's XML configuration was not designed for quick simultaneous writes like a traditional database. It may be necessary to delay API calls in sequence to prevent unexpected behavior. Alternatively, you may limit the API to read-only mode to only allow endpoints with read (GET) access within the webConfigurator's System > API page.
...
---
[Back to top](#pfsense-rest-api-documentation)
>>>>>>> 60f2fea (OpenVPN prototype initial commit)
>>>>>>> c9c1d7f (OpenVPN prototype initial commit)
