...
class APIE2ETestServicesIPsecPhase1(e2e_test_framework.APIE2ETest):
    ...
    get_privileges = ["page-all", "page-vpn-ipsec-editphase1"]
    post_privileges = ["page-all", "page-vpn-ipsec-editphase1"]
    put_privileges = ["page-all", "page-vpn-ipsec-editphase1"]
    delete_privileges = ["page-all", "page-vpn-ipsec-editphase1"]
    ...
    delete_tests = [
        ...
        },
        {
            ...
            "req_data": {"ikeid": 2, "apply": True}
        },
        {
            "name": "Delete cert used for IPsec testing",
            "uri": "/api/v1/system/certificate",
            "req_data": {"descr": "IPSEC_CERT_RSA"}
        },
        {
            "name": "Delete CA used for IPsec testing",
            "uri": "/api/v1/system/ca",
            "req_data": {"descr": "IPSEC_CA_RSA"}
        }
    ]
    ...
    def post_post(self):
        ...
        if len(self.post_responses) == 1:
            ...
            for test in self.put_tests:
                if "needs_caref" in test:
                    self.put_tests[counter]["req_data"]["caref"] = self.post_responses[0]["data"]["refid"]
                counter = counter + 1
        ...
        if len(self.post_responses) == 2:
            ...
            for test in self.post_tests:
                if "needs_certref" in test:
                    self.post_tests[counter]["req_data"]["certref"] = self.post_responses[1]["data"]["refid"]
                counter = counter + 1

            counter = 0
            for test in self.put_tests:
                if "needs_certref" in test:
                    self.put_tests[counter]["req_data"]["certref"] = self.post_responses[1]["data"]["refid"]
                counter = counter + 1
